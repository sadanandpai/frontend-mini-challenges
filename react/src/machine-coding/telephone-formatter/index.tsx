...
export default function TelephoneFormatter() {
  ...
  const getStringWithNumbersOnly = (str: string) => [...str].filter(v => Number.isInteger(+v) && v !== ' ').join('');

  const formatString = (e: ChangeEvent<HTMLInputElement>) => {
    const numStr = getStringWithNumbersOnly(e.target.value);
    setPhoneValue(numStr.length > 3 ? '+(' + numStr.substring(0, 3) + ') - ' + numStr.substring(3) : numStr);
  };
